...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','kuanshixinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','kuanshixinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('kuanshixinxi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusskuanshixinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('kuanshixinxi', this.detail);
				this.$utils.msg('审核成功');
