...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','chimaxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','chimaxinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('chimaxinxi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusschimaxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('chimaxinxi', this.detail);
				this.$utils.msg('审核成功');
