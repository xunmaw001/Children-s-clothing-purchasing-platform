<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.kuanshi" type="text" placeholder="款式" ></input>
			</view>
                  			<view v-if="queryIndex==2" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.chima" type="text" placeholder="尺码" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"0","boxShadow":" 0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"0 2% 20rpx 0","borderColor":"red","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"48%","borderStyle":"solid","height":"auto"}'>
                                                                                                                                                      <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"200rpx"}' mode="aspectFill" :src="product.tupian?baseUrl+product.tupian.split(',')[0]:''"></image>
                                                                                                                                                                                                                                                                              <view :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.shangpinmingcheng}}</view>
            ...
            <text v-if="userid && isAuth('shangpinxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('shangpinxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('shangpinxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('shangpinxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
        ...
        <view  @tap="onDetailTap(product)" v-if="index%6==4" class="list-item" :style='{"padding":"0","boxShadow":" 0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"0 0 20rpx","borderColor":"red","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                    ...
                                    <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.shangpinmingcheng}}</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.kuanshi="";
				this.searchForm.chima="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.kuanshi){
					params['kuanshi'] = '%' + this.searchForm.kuanshi + '%'
				}
				if(this.searchForm.chima){
			...
			async search(){
				...
				if(this.searchForm.kuanshi){
					searchForm['kuanshi'] = '%' + this.searchForm.kuanshi + '%'
				}
				if(this.searchForm.chima){
					searchForm['chima'] = '%' + this.searchForm.chima + '%'
				}
				if(this.searchForm.pinpai){
