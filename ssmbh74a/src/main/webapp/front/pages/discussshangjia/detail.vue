...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshangjiaCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshangjiaCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshangjia');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussshangjia');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshangjia', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshangjia', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussshangjia/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshangjia', this.detail);
				this.$utils.msg('审核成功');
